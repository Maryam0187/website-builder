  ...
  const address =
    features?.technonaireAddress === "random"
      ...
      : features?.technonaireAddress === "chosen"
        ...
        : null;
  return [
    features?.live ? "Live website" : null,
    features?.hosting ? "Hosting" : null,
    address,
    features?.domain ? "Custom domain" : null,
    features?.pwa ? "PWA app" : null,
    features?.premiumTemplates ? "All templates" : null,
    features?.templateSwitch ? "Switch templates" : null,
  ].filter(Boolean);
}
...
export default function PackageFlow({
  ...
  const oneTimeAddons = (billing?.shopAddons || billing?.addons || []).filter(
    (item) =>
      item &&
      !monthlyPlanIds.has(item.id) &&
      item.id !== "free" &&
      item.kind !== "plan" &&
                ...
                <p className="mt-1 text-sm text-blue-100">
                  {canceling
                    ...
                    : paidActive
                      ...
                      : "Default package · $0 · 1 website · all templates · switch anytime. Add-ons below for live hosting or a second site."}
                </p>
          ...
          <div className="space-y-3">
            <div>
              ...
              <p className="mt-1 text-sm text-blue-100">
                ...
                Pro + PWA ($39) to publish and host. Add a 2nd website on any plan — Free $5/mo,
                Starter $7/mo, Custom $15/mo, Domain $24/mo, Pro $34/mo. Already subscribed?
                Upgrade and pay only the difference now.
              </p>
            </div>
              ...
              {shopPackages.map((item) => {
                ...
                return (
                  <AddonCard
                    key={item.id}
                    item={item}
                    badge={isUpgrade ? "Upgrade" : "Monthly"}
                    ...
                    onAction={() => {
                      if (isCurrent && canceling) {
                        if (
                          !window.confirm(
                            `Keep ${item.name}? We’ll stop the cancellation. Billing continues on this plan.`,
                          )
                        ) {
                          return;
                        }
                      ...
                      if (isUpgrade) {
                        if (
                          !window.confirm(
                            canceling
                              ? `Upgrade to ${item.name}? We’ll stop the cancellation. You’ll pay ${formatMoney(differenceCents)} now (the difference). Starting next month you’ll be billed ${item.priceLabel} (additional sites ${item.additionalPriceLabel || "discounted"}).`
                              : `Upgrade to ${item.name}? You’ll pay ${formatMoney(differenceCents)} now (the difference). Starting next month you’ll be billed ${item.priceLabel} (additional sites ${item.additionalPriceLabel || "discounted"}).`,
                          )
                        ) {
                          return;
                        }
                        ...
                        isDowngrade &&
                        !window.confirm(
                          `Switch to ${item.name} (${item.priceLabel})? You’ll pay for this package in Stripe.`,
                        )
                      ) {
                        return;
                      }
          ...
          <div className="space-y-4">
            ...
            {canCreateSite ? (
              ...
              <button
                ...
                disabled={busy}
                onClick={() => {
                  const name = window.prompt("Name for the new website?", "My second website");
                  if (name == null) return;
                  onAction("create-site", null, { brandName: name.trim() || "My second website" });
                }}
